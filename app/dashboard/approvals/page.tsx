    ...
    <div style={{ minHeight:"100vh", background:"#F5F6FA", fontFamily:"'Outfit',sans-serif", color:"#12111A", display:"flex" }}>
      ...
      <main className="main" style={{position:"relative",zIndex:1}}>
        ...
        <div className="fi fi1" style={{marginBottom:"28px"}}>
          <h1 style={{fontSize:"clamp(22px,3vw,28px)",fontWeight:800,letterSpacing:"-0.6px",marginBottom:"5px",color:"#12111A"}}>Approvals</h1>
          <p style={{fontSize:"13px",color:"#6B6B7A"}}>Track client feedback and approvals across all your projects</p>
        </div>
        ...
        ) : needsAction.length === 0 && recentlyDone.length === 0 ? (
          <div className="fi fi2" style={{display:"flex",justifyContent:"center",marginTop:"20px"}}>
            <div style={{background:"#ffffff",border:"1.5px dashed rgba(11,171,108,0.2)",borderRadius:"16px",padding:"56px 24px",textAlign:"center",maxWidth:"380px",width:"100%"}}>
              ...
              <div style={{fontSize:"16px",fontWeight:800,color:"#12111A",marginBottom:"6px"}}>No approvals yet</div>
              <div style={{fontSize:"13px",color:"#8A8A9A",lineHeight:1.6,maxWidth:"300px",margin:"0 auto 20px"}}>Request client approval on a project stage and their response will appear here.</div>
              <button onClick={() => router.push("/dashboard")} style={{background:"#fff",color:"#6B6B7A",border:"1px solid #E4E4E8",borderRadius:"10px",padding:"10px 20px",fontSize:"13px",fontWeight:600,cursor:"pointer"}}>View Projects</button>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="fi fi2" style={{marginBottom:"32px"}}>
              <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"14px"}}>
                ...
                <h2 style={{fontSize:"12px",fontWeight:700,color:"#F59E0B",textTransform:"uppercase",letterSpacing:"0.07em"}}>Needs your attention</h2>
                {needsAction.length > 0 && (
                  <span style={{background:"rgba(245,158,11,0.15)",color:"#F59E0B",fontSize:"11px",fontWeight:700,borderRadius:"8px",padding:"2px 8px"}}>{needsAction.length}</span>
                )}
              </div>

              {needsAction.length === 0 ? (
                <div style={{padding:"28px",textAlign:"center",border:"1px dashed #E4E4E8",borderRadius:"16px",color:"#6B6B7A"}}>
                  <div style={{fontSize:"22px",marginBottom:"8px",opacity:0.4}}>✓</div>
                  <div style={{fontSize:"13px",fontWeight:600,color:"#6B6B7A"}}>All clear — no pending revisions</div>
                </div>
              ) : (
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                  {needsAction.map(({project,stage,lastActivity}) => (
                    <div key={`${project.id}-${stage.id}`} className="action-card" onClick={() => router.push(`/dashboard/project/${project.id}`)}>
                      {/* Project avatar */}
                      <div style={{width:"44px",height:"44px",borderRadius:"12px",background:"rgba(245,158,11,0.12)",border:"1px solid rgba(245,158,11,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"13px",fontWeight:800,color:"#F59E0B",flexShrink:0,marginLeft:"6px"}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"4px",flexWrap:"wrap"}}>
                          ...
                          <span style={{fontSize:"11px",color:"#6B6B7A"}}>→</span>
                          <span style={{fontSize:"13px",color:"#6B6B7A",fontWeight:500}}>{stage.title}</span>
                        </div>
                        <div style={{fontSize:"12px",color:"#6B6B7A",marginBottom:lastActivity?.message?"8px":0}}>
                          {project.client_name} · {lastActivity ? timeAgo(lastActivity.created_at) : ""}
                        </div>
                        {lastActivity?.message && (
                          <div style={{fontSize:"13px",color:"#4A4A5A",background:"rgba(245,158,11,0.07)",border:"1px solid rgba(245,158,11,0.15)",borderRadius:"8px",padding:"8px 12px",lineHeight:1.5}}>
                            <span style={{color:"#F59E0B",marginRight:"6px"}}><Icons.Warning /></span>
                            "{lastActivity.message}"
                          </div>
                        )}
                      </div>
            ...
            <div className="fi fi3">
              <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"14px"}}>
                ...
                <h2 style={{fontSize:"12px",fontWeight:700,color:"#0BAB6C",textTransform:"uppercase",letterSpacing:"0.07em"}}>Recently approved</h2>
              ...
              {recentlyDone.length === 0 ? (
                <div style={{padding:"28px",textAlign:"center",border:"1px dashed #E4E4E8",borderRadius:"16px",color:"#6B6B7A"}}>
                  <div style={{fontSize:"13px",fontWeight:600,color:"#6B6B7A"}}>No approvals in the last 2 weeks</div>
                </div>
              ) : (
