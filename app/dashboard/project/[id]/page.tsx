...
export default function ProjectDetailPage() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080a18", fontFamily:"'Outfit',sans-serif", color:"#fff" }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .pill-btn {
          ...
          transition:all 0.15s ease; white-space:nowrap; border:1px solid transparent;
        }
        ...
        .approval-btn {
          ...
          color:#fff; transition:opacity 0.2s, transform 0.15s; width:100%;
        ...
        @media (max-width:768px) {
          ...
          .main-content { margin-left:0 !important; padding:80px 16px 40px !important; }
          .header-card-inner { flex-direction:column !important; }
          .header-actions { flex-direction:row !important; flex-wrap:wrap; width:100% !important; }
          .status-card { min-width:unset !important; flex:1; }
          .progress-label { flex-direction:column !important; align-items:flex-start !important; gap:2px !important; }
          .stage-header { flex-direction:column !important; align-items:flex-start !important; }
          .pill-row { flex-wrap:wrap !important; }
          .stage-tabs-scroll { gap:6px !important; }
        }
      `}</style>
      ...
      {/* ── Main ── */}
      <main className="main-content" style={{ position:"relative", zIndex:1, minHeight:"100vh" }}>
        ...
        }} id="desktop-back">← Back to Studio</button>
        <style>{`@media(max-width:768px){#desktop-back{display:none!important}}`}</style>

        {/* ── Header card ── */}
        <div className="fi fi1" style={{
          background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.08)",
          borderRadius:"20px", padding:"24px", marginBottom:"24px", backdropFilter:"blur(12px)",
        }}>
          <div className="header-card-inner" style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:"16px", marginBottom:"20px" }}>
            <div style={{ flex:1, minWidth:0 }}>
              <h1 style={{ fontSize:"clamp(20px,4vw,28px)", fontWeight:800, letterSpacing:"-0.5px", marginBottom:"6px" }}>
                {project?.name}
              </h1>
              <div style={{ display:"flex", alignItems:"center", gap:"12px", flexWrap:"wrap" }}>
                <span style={{ fontSize:"14px", color:"rgba(255,255,255,0.55)", fontWeight:500 }}>{project?.client_name}</span>
                {project?.client_email && (
                  <span style={{ fontSize:"13px", color:"rgba(255,255,255,0.25)" }}>{project.client_email}</span>
                )}
              </div>
            </div>
            <div className="header-actions" style={{ display:"flex", alignItems:"center", gap:"10px", flexShrink:0 }}>
              {ps && project && (
                <div className="status-card" style={{
                  ...
                  <div style={{ fontSize:"10px", color:"rgba(255,255,255,0.35)", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.08em" }}>Project</div>
                  <div style={{ display:"flex", alignItems:"center", gap:"5px", fontSize:"12px", fontWeight:700, color:ps.color }}>
                    <span style={{ width:"6px", height:"6px", borderRadius:"50%", background:ps.color }} />
                    {ps.label}
                  </div>
                  <div style={{ fontSize:"10px", color:"rgba(255,255,255,0.2)" }}>tap to change</div>
                </div>
              )}
              {/* Desktop share btn */}
              <button className={`share-btn${copied?" copied":""}`} onClick={copyPortalLink} id="desktop-share">
                <span>{copied?"✓":"⬡"}</span>{copied?"Copied!":"Share portal"}
              </button>
              <style>{`@media(max-width:768px){#desktop-share{display:none!important}}`}</style>
            </div>
          </div>

          {/* Progress */}
          <div>
            <div className="progress-label" style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"8px", gap:"8px" }}>
              <span style={{ fontSize:"11px", color:"rgba(255,255,255,0.4)", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.06em" }}>Progress</span>
              <span style={{ fontSize:"12px", fontWeight:700, color:progressPct===100?"#0BAB6C":"rgba(255,255,255,0.6)" }}>
                {completeCount}/{stages.length} stages · {progressPct}%
              </span>
            </div>
            <div style={{ height:"6px", background:"rgba(255,255,255,0.07)", borderRadius:"99px", overflow:"hidden" }}>
              <div style={{
                ...
                background:progressPct===100?"linear-gradient(90deg,#0BAB6C,#5B4CF5)":"linear-gradient(90deg,#5B4CF5,#7B6CF9)",
        ...
        {/* Error */}
        {error && (
          <div style={{ background:"rgba(232,93,117,0.1)", border:"1px solid rgba(232,93,117,0.3)", borderRadius:"10px", padding:"14px 18px", marginBottom:"20px", fontSize:"14px", color:"#E85D75" }}>
            ⚠️ {error}
          </div>
        )}
        ...
        {/* ── Stage tabs ── */}
        {stages.length > 0 && (
          <div className="fi fi2 stage-tabs-scroll" style={{ display:"flex", alignItems:"center", overflowX:"auto", paddingBottom:"4px", marginBottom:"20px", gap:"0", scrollbarWidth:"none" }}>
            <style>{`.stage-tabs-scroll::-webkit-scrollbar{display:none}`}</style>
            {stages.map((stage, i) => {
              ...
              return (
                <div key={stage.id} style={{ display:"flex", alignItems:"center" }}>
                  <button className={`stage-tab${isActive?" active":""}`} onClick={() => setActiveStageId(stage.id)}>
                    ...
                    {fc>0 && <span style={{ background:"rgba(91,76,245,0.25)", color:"#a093ff", fontSize:"10px", fontWeight:700, borderRadius:"10px", padding:"1px 6px" }}>{fc}</span>}
                  </button>
                  {i<stages.length-1 && <div style={{ width:"16px", height:"1px", background:"rgba(255,255,255,0.08)", flexShrink:0 }} />}
                </div>
              );
            })}
          </div>
        ...
        {stages.length===0 && !loading && (
          <div className="fi fi2" style={{ background:"rgba(255,255,255,0.03)", border:"1px dashed rgba(255,255,255,0.1)", borderRadius:"16px", padding:"48px", textAlign:"center", color:"rgba(255,255,255,0.3)", marginBottom:"20px" }}>
        ...
        {/* ── Active stage panel ── */}
        {activeStage && (
          <div className="fi fi3" style={{ background:"rgba(255,255,255,0.022)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:"20px", padding:"24px", backdropFilter:"blur(12px)" }}>

            {/* Stage header */}
            <div className="stage-header" style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:"16px", marginBottom:"20px" }}>
              <div style={{ flex:1 }}>
                <h2 style={{ fontSize:"clamp(16px,3vw,20px)", fontWeight:800, marginBottom:"10px", letterSpacing:"-0.3px" }}>
                  {activeStage.title}
                </h2>
                <div className="pill-row" style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
                  {(["not_started","in_progress","complete"] as StageStatus[]).map(s => {
                    ...
                    return (
                      <button key={s} className="pill-btn" onClick={() => updateStageStatus(activeStage.id, s)} style={{
                        background:isSel?cfg.bg:"rgba(255,255,255,0.04)",
                        border:`1px solid ${isSel?cfg.border:"rgba(255,255,255,0.08)"}`,
              ...
              {/* Approval btn — desktop inline */}
              <button className={`approval-btn${approvalSent===activeStage.id?" sent":""}`}
                onClick={requestApproval} id="desktop-approval"
                style={{ width:"auto", flexShrink:0 }}>
                {approvalSent===activeStage.id?<><span>✓</span> Sent!</>:<><span>⬡</span> Request approval</>}
              </button>
              <style>{`@media(max-width:768px){#desktop-approval{display:none!important}}`}</style>
            </div>

            {/* Mobile approval btn */}
            <div id="mobile-approval" style={{ marginBottom:"20px" }}>
              <button className={`approval-btn${approvalSent===activeStage.id?" sent":""}`} onClick={requestApproval}>
                {approvalSent===activeStage.id?<><span>✓</span> Approval requested!</>:<><span>⬡</span> Request client approval</>}
              </button>
            </div>
            <style>{`@media(min-width:769px){#mobile-approval{display:none!important}}`}</style>
            ...
            <div style={{ marginBottom:"28px" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"10px" }}>
                <h3 style={{ fontSize:"13px", fontWeight:700, color:"rgba(255,255,255,0.7)", textTransform:"uppercase", letterSpacing:"0.06em" }}>Stage Notes</h3>
            ...
            <div>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"14px" }}>
                <h3 style={{ fontSize:"13px", fontWeight:700, color:"rgba(255,255,255,0.7)", textTransform:"uppercase", letterSpacing:"0.06em" }}>
                  Files {activeStageFiles.length>0&&<span style={{ marginLeft:"6px", fontSize:"12px", color:"rgba(255,255,255,0.3)", fontWeight:400, textTransform:"none", letterSpacing:0 }}>{activeStageFiles.length} file{activeStageFiles.length!==1?"s":""}</span>}
                </h3>
              ...
              <div className={`upload-zone${dragOver?" drag":""}`}
                ...
                {uploading?(
                  ...
                  <span style={{ fontSize:"12px", color:"rgba(255,255,255,0.25)" }}>Images, PDFs, Figma exports, ZIPs</span></>
                )}
              </div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
                  {activeStageFiles.map(file=>(
                    <div key={file.id} className="file-row">
                      <div style={{ width:"36px", height:"36px", borderRadius:"9px", background:"rgba(91,76,245,0.1)", border:"1px solid rgba(91,76,245,0.18)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"16px", flexShrink:0 }}>{fileIcon(file.name)}</div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontSize:"13px", fontWeight:600, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", color:"rgba(255,255,255,0.9)" }}>{file.name}</div>
                        <div style={{ fontSize:"11px", color:"rgba(255,255,255,0.28)", marginTop:"2px" }}>{file.file_size?formatBytes(file.file_size):""} · {timeAgo(file.created_at)}</div>
                      </div>
                      <a href={file.file_url} target="_blank" rel="noopener noreferrer" style={{ padding:"6px 12px", borderRadius:"7px", border:"1px solid rgba(255,255,255,0.09)", background:"rgba(255,255,255,0.04)", color:"rgba(255,255,255,0.55)", fontSize:"12px", fontWeight:600, textDecoration:"none", whiteSpace:"nowrap", fontFamily:"'Outfit',sans-serif" }}>View →</a>
                    </div>
                  ))}
                </div>
              )}
