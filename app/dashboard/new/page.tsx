  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080a18", fontFamily:"'Outfit',sans-serif", color:"#fff" }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .stage-input {
          ...
          color:#fff; font-family:'Outfit',sans-serif; font-size:14px; padding:12px 14px;
        }
        ...
        .topbar  { display:none; }
        .main-wrap { margin-left:220px; min-height:100vh; display:flex; align-items:center; justify-content:center; padding:40px; }

        @media (max-width:768px) {
          .sidebar  { display:none !important; }
          .topbar   { display:flex !important; }
          .main-wrap { margin-left:0 !important; padding:80px 16px 40px !important; align-items:flex-start !important; }
          .card-inner { padding:24px !important; }
      ...
      <aside className="sidebar" style={{
        ...
        <nav style={{ display:"flex", flexDirection:"column", gap:"4px", flex:1 }}>
          {[
            ...
            <div key={label} className="nav-item" onClick={() => path && router.push(path)}>
      ...
      <header className="topbar" style={{
        ...
        <button onClick={() => step===2 ? setStep(1) : router.push("/dashboard")} style={{
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"6px" }}>
          {[1,2].map(s => (
            <div key={s} style={{
              ...
              background: step===s?"linear-gradient(135deg,#5B4CF5,#0BAB6C)":step>s?"#0BAB6C":"rgba(255,255,255,0.1)",
              display:"flex", alignItems:"center", justifyContent:"center",
        ...
        <div style={{ width:"100%", maxWidth:"520px" }}>
          ...
          <div className="fi fi1" style={{ display:"flex", alignItems:"center", gap:"16px", marginBottom:"28px" }}>
            <button onClick={() => step===2 ? setStep(1) : router.push("/dashboard")} style={{
              ...
              display:"flex", alignItems:"center", justifyContent:"center",
            }} id="desktop-back">←</button>
            <style>{`@media(max-width:768px){#desktop-back{display:none!important}}`}</style>
            <div>
              <h1 style={{ fontSize:"clamp(20px,4vw,24px)", fontWeight:800, letterSpacing:"-0.5px", lineHeight:1.1 }}>
          ...
          <div className="fi fi1" style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"28px" }} id="desktop-steps">
            <style>{`@media(max-width:768px){#desktop-steps{display:none!important}}`}</style>
            {[1,2].map(s => (
              <div key={s} style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                  <div style={{
                    ...
                    color:step>=s?"#fff":"rgba(255,255,255,0.3)", transition:"all 0.3s",
                  }}>{step>s?"✓":s}</div>
                  <span className="step-label" style={{ fontSize:"13px", fontWeight:step===s?600:400, color:step===s?"#fff":"rgba(255,255,255,0.35)" }}>
          ...
          {/* Card */}
          <div className="fi fi2 card-inner" style={{
            ...
            {step===1 && (
              <div style={{ display:"flex", flexDirection:"column", gap:"20px" }}>
                ...
                <div>
                  <label style={{ display:"block", fontSize:"11px", fontWeight:700, color:"rgba(255,255,255,0.4)", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:"8px" }}>
                    Client Email
                    <span style={{ marginLeft:"6px", fontWeight:400, textTransform:"none", letterSpacing:0, color:"rgba(255,255,255,0.25)", fontSize:"11px" }}>— optional</span>
                  </label>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"16px" }}>
                  ...
                  {stages.map((s, i) => (
                    <div key={s+i} className="stage-pill"
                      ...
                      <div style={{ display:"flex", gap:"2px" }}>
                        ...
                        <button className="icon-btn" onClick={()=>removeStage(i)} style={{ color:"rgba(232,93,117,0.5)" }}>✕</button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="add-stage-row">
                  ...
                  <button onClick={addStage} style={{
                    padding:"12px 18px", background:"rgba(91,76,245,0.2)",
                ...
                <div style={{ background:"rgba(91,76,245,0.06)", border:"1px solid rgba(91,76,245,0.15)", borderRadius:"12px", padding:"16px" }}>
                  <div style={{ fontSize:"11px", color:"rgba(255,255,255,0.3)", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:"8px" }}>Summary</div>
                  ...
                  <div style={{ fontSize:"13px", color:"rgba(255,255,255,0.3)" }}>{stages.length} stage{stages.length!==1?"s":""}</div>
                </div>

                <button className="primary-btn" disabled={loading||stages.length===0} onClick={handleCreate}>
                  {loading?(
                    <><div style={{ width:"16px", height:"16px", border:"2px solid rgba(255,255,255,0.3)", borderTopColor:"#fff", borderRadius:"50%", animation:"spin 0.7s linear infinite" }} />Creating project…</>
                  ):"Create Project →"}
                </button>
              </div>
            )}
